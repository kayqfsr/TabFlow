...
describe('getSaveValidationError', () => {
  ...
  test('returns an error message for values below the minimum', () => {
    expect(getSaveValidationError(2)).toBe('Por favor, escolha um valor entre 3 e 10.');
  });

  test('returns an error message for values above the maximum', () => {
    expect(getSaveValidationError(11)).toBe('Por favor, escolha um valor entre 3 e 10.');
  });

  test('returns an error message for non-numeric values', () => {
    expect(getSaveValidationError(NaN)).toBe('Por favor, escolha um valor entre 3 e 10.');
  });
});
